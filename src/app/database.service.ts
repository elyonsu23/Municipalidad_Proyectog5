...
export class DatabaseService {
  ...
  //METODOS DE PARTICIPANTES

  // Método para crear un nuevo grupo (POST)
  altaParticipante(participanteData: any): Observable<any> {
    const headers = this.createHeaders();
    return this.http.post(`${this.apiUrl}?entity=participantes`, participanteData, { headers });
  }

  // Método para recuperar todos los grupos desde la base de datos (GET)
  recuperarParticipantes(): Observable<any> {
    const headers = this.createHeaders();
    return this.http.get(`${this.apiUrl}?entity=participantes`);
  }

  // Método para eliminar un grupo de la base de datos (DELETE)
  bajaParticipantes(Id_Participante: number): Observable<any> {
    const headers = this.createHeaders();
    return this.http.delete(`${this.apiUrl}?entity=participantes&id=${Id_Participante}`, { headers });
  }

  // Método para modificar un grupo existente en la base de datos (PUT)
  modificarParticipantes(participante: any, Id_Participante: any): Observable<any> {
    const headers = this.createHeaders();
    return this.http.put(`${this.apiUrl}?entity=participantes&id=${Id_Participante}`, participante, { headers });
  }
}
