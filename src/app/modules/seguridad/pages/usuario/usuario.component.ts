...
export class UsuarioComponent implements OnInit {
  ...
  recuperarGrupos() {
    this.databaseService.recuperarGrupos().subscribe({
      next: (response) => {
        if (Array.isArray(response)) {
          this.grupos = response;  // Aquí debes tener un array de objetos que contengan idGrupo y descripcion
        } else {
  ...
  obtenerDescripcionGrupo(grupoId: any): string {
    const grupo = this.grupos.find(g => g.idGrupo === grupoId);
    return grupo ? grupo.descripcion : 'Sin grupo'; // Si no encuentra el grupo, muestra un mensaje
  }
